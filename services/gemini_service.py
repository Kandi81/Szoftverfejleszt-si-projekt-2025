...
class GeminiService:
    ...
    def _load_api_key_from_file(self, path: str = "resource/gemini_api_key.txt") -> Optional[str]:
        ...
        Args:
            path: Path to API key file (plain text file with just the key)
        ...
        if os.path.exists(path):
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    key = f.read().strip()
                    if key and key.startswith('AIza'):  # Gemini keys start with AIza
                        print(f"[GEMINI] API key loaded from {path}")
                        return key
                    else:
                        print(f"[GEMINI] Invalid API key format in {path}")
            except Exception as e:
                print(f"[GEMINI] Error reading API key from {path}: {e}")
        else:
            print(f"[GEMINI] API key file not found: {path}")
        return None
